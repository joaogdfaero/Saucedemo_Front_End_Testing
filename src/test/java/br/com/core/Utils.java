...
public interface Utils {
	...
	public default boolean isStringInPage(String string) {
		try {
			return getDriver().getPageSource().contains(string);
		} catch (Exception e) {
			return false;
		}
	}
...
//	    System.out.println("opção escolhido eh: "+options.get(indexToSelect).getText());
	...
	public default String getText(By by) {
		...
		try {
			...
			text = object.getText();
			ReportGenerator.logStatus(Status.PASS, "Achou o elemento");
			return text;
		} catch (Exception e) {
			ReportGenerator.logStatus(Status.FAIL, "Não achou o elemento");
		}
	...
	default boolean isOnScreen(By by, int timeoutInSeconds) {	
		try {
